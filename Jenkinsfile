pipeline {
    ...
    stages {
        stage('git checkout') {
            steps {
                git branch: 'master', url: 'https://github.com/divekarsiddhi/Ekart.git'
            }
        }
        ...
        stage('build and Tag docker image') {
            steps {
                script {
                        sh "docker build -t divekarsiddhi/ekart:latest -f docker/Dockerfile ."
                    }
            }
        }
            ...
            steps{
                script{
                   withCredentials([string(credentialsId: 'dockerhub-pwd', variable: 'dockerhubpwd')]) {
                    sh "dckr_pat_dk4RwBEvHoAm-l_UEaMo7_kYjNg" | docker login -u divekarsiddhi --password-stdin
